...
namespace Owner.API.Controllers
{
    ...
    public class OwnerController : ControllerBase
    {
        ...
        public IActionResult CreateOwner(Model.Owner owner)
        {
            if (owner == null)
            {
                throw new BadHttpRequestException("No data to add.");
                //return BadRequest();
            }
            else
            {
                ...
                {
                    throw new BadHttpRequestException("You cannot write 'hack' words in the description.");
        ...
        public IActionResult UpdateOwner(int id, Model.Owner owner)
        {
            if (id != owner.Id)
            {
                throw new BadHttpRequestException("Id's did not match.");
        ...
        public IActionResult DeleteOwner(int id)
        {
            ...
            if (ownerData == null)
            {
                throw new BadHttpRequestException($"Owner with id {id} not found.");
